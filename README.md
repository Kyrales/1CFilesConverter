...
## 🎨 GUI для конвертации (NEW!)

**Графический интерфейс в стиле Cyberpunk для удобной работы с конвертацией!**
...
2. Запустите GUI:
   ```
   Двойной клик: projects/run_gui.cmd
   ```

3. Выберите проекты, нажмите "ВЫПОЛНИТЬ" и наблюдайте за прогрессом!

### Возможности GUI

✅ Автоматическое сканирование проектов
✅ Выбор проектов чекбоксами
...
- 📘 [projects/GUI_README.md](projects/GUI_README.md) - Полная документация
- 🚀 [projects/QUICK_START.md](projects/QUICK_START.md) - Краткая инструкция
- 🔧 [projects/INSTALLATION.md](projects/INSTALLATION.md) - Установка и настройка

---

## Python-обертка для конвертации

В каталоге [projects](./projects) расположен Python-скрипт [convert.py](./projects/convert.py), который упрощает работу с конвертацией файлов 1С.

### Использование convert.py

```bash
# Конвертация проекта с .env файлами
python convert.py --env F:\Projects\MyProject

# Конвертация с переопределением пути назначения
python convert.py --env F:\Projects\MyProject --output F:\Output

# Если .env файлы находятся в той же папке, что и convert.py
python convert.py
```

### Особенности

- Автоматически объединяет `base.env` и `.env` файлы проекта
- Поддерживает все типы конвертации (определяются по `ScriptName` в .env)
- CMD скрипты берутся из папки `projects/scripts/`
- Валидирует пути в зависимости от типа конвертации (файл/каталог)
- Цветной вывод в консоль (Windows)

### Структура проекта для convert.py

```
projects/
├── base.env                    # Базовые настройки (опционально)
├── scripts/                    # CMD скрипты конвертации
│   ├── dp2epf.cmd
│   ├── conf2cf.cmd
│   └── ...
├── convert.py                  # Python-обертка
└── MyProject/                  # Папка проекта
    ├── project.env             # Настройки проекта
    └── ...
```

### Переменные в .env для convert.py

- **ScriptName** - имя CMD скрипта из `projects/scripts/` (например: `dp2epf.cmd`)
- **V8_SRC_PATH** - путь к источнику
- **V8_DST_PATH** - путь назначения (можно переопределить через `--output`)
- Все остальные переменные из [раздела "Переменные среды"](#environment)
